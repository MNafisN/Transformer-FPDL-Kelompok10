...
def train():
    batch_size = 64
    train_dataloader = torch.utils.data.DataLoader(ListDataset(['ListDataset.txt']), batch_size=batch_size, shuffle=True, num_workers=0)
    val_dataloader = torch.utils.data.DataLoader(ListDataset('TestDataset.txt'), batch_size=batch_size, shuffle=False, num_workers=0)
    model = make_model(len(char2token))
    model.load_state_dict(torch.load('saved_model/trained_resnet.pth'))
    model.cuda()
